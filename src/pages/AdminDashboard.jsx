...
import { getToolAnalytics } from '../utils/analyticsTracker';
import {
  ShieldCheck,
  Store,
  Printer,
...
export default function AdminDashboard() {
  ...
  // Login form state (Empty defaults for security)
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center space-x-2 flex-wrap">
            ...
            >
              ...
              <span>Tool Analytics &amp; Activity</span>
            </button>
        ...
        {/* TAB 2: TOOL ANALYTICS & USAGE ACTIVITY */}
        {activeTab === 'ANALYTICS' && (() => {
          const analyticsData = getToolAnalytics();
          const toolList = Object.values(analyticsData).sort((a, b) => b.count - a.count);
          const maxCount = toolList[0]?.count || 1;

          const mostUsed = toolList.slice(0, 8);
          const leastUsed = toolList.slice(-6).reverse();

          // Category distribution
          const catCounts = toolList.reduce((acc, t) => {
            acc[t.category] = (acc[t.category] || 0) + t.count;
            return acc;
          }, {});

          const totalExecutions = toolList.reduce((sum, t) => sum + t.count, 0);
            ...
            <div className="space-y-8 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="glass-card p-5 rounded-2xl border border-slate-800 space-y-1">
                  <div className="flex items-center justify-between text-slate-400">
                    <span className="text-xs font-bold">TOTAL TOOL EXECUTIONS</span>
                    <BarChart3 className="w-5 h-5 text-amber-400" />
                  </div>
                  <h3 className="text-3xl font-extrabold text-white">{totalExecutions}</h3>
                  <p className="text-[11px] text-emerald-400 font-bold">100% Client-Side Private Usage</p>
                </div>

                <div className="glass-card p-5 rounded-2xl border border-slate-800 space-y-1">
                  <div className="flex items-center justify-between text-slate-400">
                    <span className="text-xs font-bold">🔥 #1 MOST POPULAR TOOL</span>
                    <TrendingUp className="w-5 h-5 text-emerald-400" />
                  </div>
                  <h3 className="text-xl font-black text-emerald-400 truncate">{mostUsed[0]?.name || 'N/A'}</h3>
                  <p className="text-[11px] text-slate-400 font-mono">{mostUsed[0]?.count || 0} total uses</p>
                </div>

                <div className="glass-card p-5 rounded-2xl border border-slate-800 space-y-1">
                  <div className="flex items-center justify-between text-slate-400">
                    <span className="text-xs font-bold">❄️ LEAST USED TOOL</span>
                    <TrendingDown className="w-5 h-5 text-rose-400" />
                  </div>
                  <h3 className="text-xl font-black text-rose-400 truncate">{leastUsed[0]?.name || 'N/A'}</h3>
                  <p className="text-[11px] text-slate-400 font-mono">{leastUsed[0]?.count || 0} total uses</p>
                </div>
              </div>

              {/* Grid: Most Used vs Least Used Tools */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-card p-6 rounded-2xl border border-slate-800 space-y-5">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <h3 className="font-extrabold text-white text-base flex items-center space-x-2">
                      <TrendingUp className="w-5 h-5 text-emerald-400" />
                      <span>Most Used Tools (Top Performing)</span>
                    </h3>
                    <span className="text-xs text-emerald-400 font-mono font-bold">Ranked by Activity</span>
                  </div>

                  <div className="space-y-4">
                    {mostUsed.map((tool, idx) => {
                      const perc = Math.round((tool.count / maxCount) * 100);
                      return (
                        <div key={tool.id} className="space-y-1.5">
                          <div className="flex items-center justify-between text-xs font-bold">
                            <span className="text-white flex items-center space-x-2">
                              <span className="w-5 text-amber-400 font-mono text-xs font-black">#{idx + 1}</span>
                              <span>{tool.name}</span>
                              <span className="px-2 py-0.5 rounded-full bg-slate-900 border border-slate-800 text-[10px] text-slate-400">
                                {tool.category}
                              </span>
                            </span>
                            <span className="text-emerald-400 font-mono font-black">{tool.count} uses</span>
                          </div>
                ...
                <div className="glass-card p-6 rounded-2xl border border-slate-800 space-y-5">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <h3 className="font-extrabold text-white text-base flex items-center space-x-2">
                      <TrendingDown className="w-5 h-5 text-rose-400" />
                      <span>Least Used Tools (Needs Promotion)</span>
                    </h3>
                    <span className="text-xs text-rose-400 font-mono font-bold">Low Activity</span>
                  </div>

                  <div className="space-y-4">
                    {leastUsed.map((tool, idx) => (
                      <div key={tool.id} className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center justify-between">
                        <div className="space-y-0.5">
                          <div className="text-xs font-extrabold text-white">{tool.name}</div>
                          <div className="text-[11px] text-slate-400 font-mono">{tool.category} • Last used: {tool.lastUsed}</div>
                        </div>
                        <div className="px-3 py-1 rounded-lg bg-rose-950/80 border border-rose-800 text-rose-300 font-mono text-xs font-black">
                          {tool.count} uses
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="pt-4 border-t border-slate-800 space-y-3">
                    <h4 className="text-xs font-black uppercase text-slate-400 flex items-center justify-between">
                      <span>Category Distribution Share</span>
                      <PieChart className="w-4 h-4 text-cyan-400" />
                    </h4>
                    <div className="grid grid-cols-2 gap-2">
                      {Object.entries(catCounts).map(([cat, cnt]) => (
                        <div key={cat} className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs flex justify-between items-center">
                          <span className="text-slate-300 font-bold">{cat}</span>
                          <span className="text-cyan-400 font-mono font-black">{Math.round((cnt / totalExecutions) * 100)}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
